...
| **`ip`** | `ip addr` | Affiche/configure les interfaces réseau (remplace *ifconfig*). || **`less`** | `less gros.log` | Affiche le contenu page par page (`q` pour quitter). |
...
| **`touch`** | `touch notes.txt` | Crée un fichier vide ou met à jour sa date. || **`tree`** | `tree` | Affiche l'arborescence des fichiers et dossiers sous forme d'arbre visuel. |
